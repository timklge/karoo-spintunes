...
class ShowPagingSource(val showId: String, val ctx: Context, val webAPIClient: WebAPIClient) : PagingSource<Int, TrackObject>() {
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, TrackObject> {
        return try {
            val nextPageNumber = params.key ?: 0
            val response = webAPIClient.getShowEpisodes(showId, nextPageNumber * WebAPIClient.PAGE_SIZE)
