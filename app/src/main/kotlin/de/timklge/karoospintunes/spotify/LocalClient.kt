...
class LocalClient(val context: Context) : APIClient {
        ...
        mutex.withLock {
            ...
            suspendCoroutine {
                ...
                future.setErrorCallback { error ->
                    Log.e(KarooSpintunesExtension.TAG, "Failed to load image: $error")
                    ...
                    callbackFlow {
                        subscription.setEventCallback { playerState ->
                            CoroutineScope(Dispatchers.IO).launch {
                                Log.d(KarooSpintunesExtension.TAG, "Player state: $playerState")
                                send(playerState)
                            }
                        }
                        ...
                        subscription.setLifecycleCallback(object : LifecycleCallback {
                            override fun onStart() {
                                Log.d(KarooSpintunesExtension.TAG, "Player subscription started")
                            }

                            override fun onStop() {
                                Log.d(KarooSpintunesExtension.TAG, "Player subscription stopped")
                                close()
                            }
                        })

                        awaitClose {
                            Log.d(KarooSpintunesExtension.TAG, "Player subscription cancelled")

                            try {
                                subscription.cancel()
                            } catch(e: Throwable){
                                Log.w(KarooSpintunesExtension.TAG, "Failed to cancel player subscription", e)
                            }
                        }
                    }.collect {
                        send(it)
    ...
    suspend fun initialize(): LocalClientConnectionState = withContext(Dispatchers.Main) {
        val connectionParams = ConnectionParams.Builder(de.timklge.karoospintunes.BuildConfig.SPOTIFY_CLIENT_ID)
            ...
            suspendCoroutine { continuation ->
                ...
                SpotifyAppRemote.connect(context, connectionParams, object : Connector.ConnectionListener {
                    override fun onConnected(appRemote: SpotifyAppRemote) {
                        ...
                        CoroutineScope(Dispatchers.IO).launch {
                            Log.d(KarooSpintunesExtension.TAG, "Connected to Spotify")
                    ...
                    override fun onFailure(throwable: Throwable) {
                        ...
                        try {
                            message = de.timklge.karoospintunes.jsonWithUnknownKeys.decodeFromString<LocalClientError>(message).message
                        } catch(e: Throwable){
                            Log.w(KarooSpintunesExtension.TAG, "Failed to parse error message", e)
                        }
            ...
            spotifyRemote?.playerApi?.playerState?.let { playerStateResult ->
                suspendCoroutine { continuation ->
                    ...
                    playerStateResult.setErrorCallback { error ->
                        Log.e(KarooSpintunesExtension.TAG, "Failed to get player state: $error")
